  ...
  export default {
    ...
    methods: {
      createdInIt() {
        this.init()
        //只在某个时间触发
        if (this.isStartNumber) {
          this.getDateNums()
        }
      },
      getDateNums({ count = 11, day = 6, num = 7 } = { ...this.yearDataNums }) {
        const arr = []
      ...
      openDisAbled: function (year, month, day) {
        ...
        if (!this.isStartNumber) {
          ...
          this.arrStartNumber.map((item) => {
            const split = item.split('-')
            const [y1, m1, d1] = [split[0], split[1], split[2]]
            const a1 = `${y1}/${m1}/${d1}`
            if (a1 === date) {
              bool = false
            }
